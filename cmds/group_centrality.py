...
class GroupCentrality():
    ...
    def run(self):
        ...
        if self.args.group_distances == None and self.args.type in ["all", "closeness"]:
            raise Error("Group distance must be required for group closeness")

        if self.args.input_file is None:
            sys.stdout.write(
                "Please specify an input file using the -i option.\n".format(self.args.input_file))
            sys.exit(1)

#todo I keep this here to scavenge this class
class KeyPlayerCOPY():
    ...
    def run(self):
        ...
        if self.args.which == 'kp-finder':
            ...
            if self.args.implementation == "greedy":
                ...
                sys.stdout.write(
                    "Implementation Error. Please contact Pyntacle developers and sent this error message, along with a command line and a log.\nQuitting.\n")
                sys.exit(1)
        ...
        if self.args.save_binary:
            ...
            if KpnegEnum.F.name in queried_stuff:
                ...
                if bf:
                    AddAttributes.add_graph_attributes(graph, k, {
                        tuple(tuple(x) for x in results[KpnegEnum.F.name][0]): results[KpnegEnum.F.name][1]})
                else:
                    AddAttributes.add_graph_attributes(graph, k, {
                        tuple(results[KpnegEnum.F.name][0]): results[KpnegEnum.F.name][1]})
            ...
            if KpnegEnum.dF.name in queried_stuff:
                ...
                if bf:
                    AddAttributes.add_graph_attributes(graph, k, {
                        tuple(tuple(x) for x in results[KpnegEnum.dF.name][0]): results[KpnegEnum.dF.name][1]})
                else:
                    AddAttributes.add_graph_attributes(graph, k, {
                        tuple(results[KpnegEnum.dF.name][0]): results[KpnegEnum.dF.name][1]})
            ...
            if KpposEnum.dR.name in queried_stuff:
                ...
                if bf:
                    ...
                    AddAttributes.add_graph_attributes(graph, k, {
                        tuple(results[KpposEnum.dR.name][0]): results[KpposEnum.dR.name][1]})
        ...
        if not self.args.no_plot and graph.vcount() < 1000:
            ...
            for metric in results:
                ...
                if metric == "F":
                    ...
                    # create a list of node colors
                    node_colors = [mreach_nodes_colour if x["name"] in results[metric][0] else other_nodes_colour for x
                                   in graph.vs()]
                    node_frames = [mreach_frames_colour if x["name"] in results[metric][0] else other_frame_colour for x
                                   in graph.vs()]
                ...
                if metric != "mreach":
                    ...
                    edge_widths = [5 if any(y in results[metric][0] for y in x["adjacent_nodes"]) else other_edge_width
                                   for
                                   x in graph.es()]
