...
class WidgetTestLogicOps(unittest.TestCase):
    def setUp(self):
        self.cleanup()
        self.graph1 = PyntacleImporter.AdjacencyMatrix(file=os.path.join(current_dir, 'test/test_sets/input/set1.txt'), sep='\t', header=True)
        self.graph2 = PyntacleImporter.AdjacencyMatrix(file=os.path.join(current_dir, 'test/test_sets/input/set2.txt'), sep='\t', header=True)

    def test_union(self):
        sys.stdout.write("Testing set union\n")
        fileout = os.path.join(current_dir, 'test/test_sets/tmp/result_set.adjm')
        expected = os.path.join(current_dir, 'test/test_sets/output/set/result_union.adjm')
        output_graph = GraphOperations.union(self.graph1, self.graph2, new_graph_name='result_set')
        PyntacleExporter.AdjacencyMatrix(graph=output_graph, output_file=os.path.join(current_dir, 'test/test_sets/tmp/result_set.adjm'),
                                         sep='\t', header=True)
    ...
    def test_intersect(self):
        ...
        fileout = os.path.join(current_dir, 'test/test_sets/tmp/result_set.adjm')
        expected = os.path.join(current_dir, 'test/test_sets/output/set/result_intersect.adjm')
    ...
    def test_difference(self):
        ...
        fileout = os.path.join(current_dir, 'test/test_sets/tmp/result_set.adjm')
        expected = os.path.join(current_dir, 'test/test_sets/output/set/result_difference.adjm')
