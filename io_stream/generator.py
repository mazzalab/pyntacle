...
from private.io_utils import generatorscanner, randomword
...
class Generator:
    ...
    def Random(params, name="Random", seed=None) -> Graph:
        ...
        :param list params: a list of two arguments. The first parameter is the number of nodes, the second parameter can be either a `float` betweeen 0 and 1 or an `int`. If it's a float , it will be considered as rewiring probability. Otherwise, it will be considered the number of edges in the Random Graph.
    ...
    def SmallWorld(params, name="SmallWorld", seed=None) -> Graph:
        ...
        #. the dimension of the lattice across all dimensions (we recommend to keep this parameter low)
        #. the distance between each node pair to consider them as connected. and #. the node rewiring probability (a `float` between 0 and 1).
