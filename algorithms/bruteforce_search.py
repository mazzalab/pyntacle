...
def crunch_fragmentation_combinations(graph: Graph, node_names_list: list, kpp_type: KpnegEnum,
    ...
    for node_names in node_names_list:
        ...
        if temp_graph.ecount() == 0:
            ...
            kppset_score_pairs_partial[node_names] = KeyPlayer.dF(graph=temp_graph, cmode=cmode)
        else:
            raise KeyError(
...
def crunch_reachability_combinations(graph: Graph, node_names_list: list, kp_type: KpposEnum, m: int,
                                     cmode: CmodeEnum) -> dict:
    kppset_score_pairs = {}
    ...
    for node_names in node_names_list:
        if kp_type == KpposEnum.mreach:
            if cmode != CmodeEnum.igraph:
                ...
                reachability_score = KeyPlayer.mreach(graph=graph, nodes=node_names, m=m,
            ...
            if cmode != CmodeEnum.igraph:
                sp_matrix = sp.get_shortestpaths(graph=graph, cmode=cmode, nodes=None)
                reachability_score = KeyPlayer.dR(graph=graph, nodes=node_names,
                                                  cmode=cmode, sp_matrix=sp_matrix)
            else:
                reachability_score = KeyPlayer.dR(graph=graph, nodes=node_names,
                                                  cmode=cmode)
...
class BruteforceSearch:
    ...
    def fragmentation(graph: Graph, k: int, metric: KpnegEnum, cmode: CmodeEnum = CmodeEnum.igraph,
                      nprocs: int = 1) -> (list, float):
        r"""
        ...
        :param int nprocs: Positive integer specifying the number of cores used to perform parallel computation. 1 is the default choice.
        ...
        if nprocs > 1:
            sys.stdout.write(u"Brute-force search of the best kp-set of size {} using {} processes\n".format(k, nprocs))
            ...
            with ProcessPoolExecutor(max_workers=nprocs) as executor:
                future_dict = {executor.submit(crunch_fragmentation_combinations, graph, chunk, metric, cmode):
    ...
    @staticmethod
    @bruteforce_search_initializer
    def reachability(graph, k: int, metric: KpposEnum, m=None, cmode=CmodeEnum.igraph, nprocs: int = 1) -> (list, float):
        r"""
        ...
        :param int nprocs: Positive integer specifying the number of processes to be spawned. 1 is the default choice.
        ...
        :raises TypeError: if ``nprocs`` is not a positive integer value greater than one
        ...
        if nprocs > 1:
            ...
            with ProcessPoolExecutor(max_workers=nprocs) as executor:
                future_dict = {executor.submit(crunch_reachability_combinations, graph, chunk, metric,
                                               m, cmode): chunk for chunk in chunks}
                for future in as_completed(future_dict):
                    chunk = future_dict[future]
                    try:
                        partial_result = future.result()
            ...
            # Do not paralellize iterations through by multi-processessing
            sys.stdout.write(u"Brute-force search of the best kp-set of size {}\n".format(k))
    ...
    def group_centrality(graph: Graph, k: int, metric: GroupCentralityEnum, cmode: CmodeEnum = CmodeEnum.igraph,
                         distance_type: GroupDistanceEnum = GroupDistanceEnum.minimum, nprocs: int = 1) -> (
            list, float):
        r"""
        ...
        :param cmodeEnum cmode: the implementation that will be used to compute the shortest paths. See :class:`~pyntacle.tools.enums.CmodeEnum`. Default is the igraph brute-force shortest path search.
        :param int nprocs: Positive integer specifying the number of processes to be spawn. 1 is the default choice
        ...
        if nprocs > 1:
            sys.stdout.write(
                u"Brute-force search of the best group of nodes of size {} using {} processes\n".format(k, nprocs))
